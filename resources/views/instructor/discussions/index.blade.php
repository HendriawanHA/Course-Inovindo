            ...
            <div class="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
                        Balas pertanyaan student dari course yang kamu kelola.
                    </p>
                </div>
                    ...
                    <select name="course_id"
                        ...
                        @foreach ($courses as $course)
                            <option value="{{ $course->id }}" @selected(request('course_id') == $course->id)>
                                {{ $course->title }} ({{ $course->discussions_count }})
                            </option>
                        @endforeach
                    </select>
                </form>
                        ...
                        <div class="flex gap-4">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
                                    {{ $discussion->course->title }} • {{ $discussion->lesson?->title ?? '-' }}
                                </p>
                ...
                @empty
                    <div class="rounded-3xl border border-dashed border-zinc-300 bg-white p-10 text-center dark:border-zinc-700 dark:bg-zinc-900">
                        ...
                        <p class="mt-2 text-sm text-zinc-500 dark:text-zinc-400">
                            Diskusi dari student akan muncul di sini.
                        </p>
                    </div>
                @endforelse
            </div>
